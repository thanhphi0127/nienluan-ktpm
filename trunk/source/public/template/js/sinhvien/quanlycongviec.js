$(document).ready(function(){
	...
	if ('addreport' == $('#flag').val()){
		...
		$('img.manipulation').show();
		$('div.bar_tilehoanthanh > div').css('width', $('#TILEHOANTHANH2').val() + '%');
		
		if ($('#POST_NGUOIDUOCGIAO_YES').val() != '') {
			var array_nguoiduocgiao = $('#POST_NGUOIDUOCGIAO_YES').val().split('   ');
				
				
			$.each (array_nguoiduocgiao, function (index, val){
				$("#NGUOIDUOCGIAO2 option").each(function(){
					if ($(this).text() == val)
	...
	*/
	$('span.manipulation').click(function() {
		...
		if ($(this).hasClass('btnAddtask')){
			...
			$('#POST_NGUOIDUOCGIAO_YES').attr('value', NGUOIDUOCGIAO);
			...
			if ('Đóng' == TEN_TRANGTHAI)
				$("fieldset.MA_TRANGTHAI > p > label > input[type=radio]").attr('value','CV_DONG');
			else if ('Mở' == TEN_TRANGTHAI)
				$("fieldset.MA_TRANGTHAI > p > label > input[type=radio]").attr('value','CV_MO');
			else if ('Trong tiến trình' == TEN_TRANGTHAI)
				$("fieldset.MA_TRANGTHAI > p > label > input[type=radio]").attr('value','CV_TRONG');	
	...
	$('img.iframe_chitietcongviec').click(function(){
		$('#iframe_chitietcongviec').hide();
	});
	
	$('a[target=iframe_chitietcongviec]').click(function(){
		
		$('div.iframe_chitietcongviec').fadeIn(300);
	});
