				...
				<?php
                    ...
                    foreach ($ds as $user)
                    {
                        ...
                        echo "<td class='sdt_gv'>
								  ...
								  <span class='txtModify' hidden>
								  <input class='sdt_gv_get sdt_gv' type='text' name='$user->mscb[sdt]' value='$user->sdt' onblur='checkSDT()' onkeyup='checkSDT()'/></span><div class='z-sdt'></div>
						  		</td>";  
					    ...
					    echo "<td class='nganhhoc'>
								  ...
								  <span class='txtModify' hidden>";
								  if (isset($LayBoMon) && !empty($LayBoMon)) {
						echo       "<select class='bomon_gv' name='$user->mscb[ma_bm]' style='width:130px;'>";
								    foreach ($LayBoMon as $temp) {
